...
<%@ taglib prefix="links" uri="http://www.vcms.com/tags/component/links"%>


<c:choose>
	<c:when test="${designComponent.componentStyle eq 'Learn'}">
		<links:learn website="${website}" websiteView="${websiteView}" page="${page}" designComponent="${designComponent}" />
	</c:when>
</c:choose>
