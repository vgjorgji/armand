...
<%@ taglib prefix="paragraphs" uri="http://www.vcms.com/tags/component/paragraphs"%>


<c:choose>
	<c:when test="${designComponent.componentStyle eq 'Learn'}">
		<paragraphs:learn website="${website}" websiteView="${websiteView}" page="${page}" designComponent="${designComponent}" />
	</c:when>
</c:choose>
