...
<%@ taglib prefix="footer" uri="http://www.vcms.com/tags/component/footer"%>


<c:choose>
	<c:when test="${designComponent.componentStyle eq 'Learn'}">
		<footer:learn website="${website}" websiteView="${websiteView}" page="${page}" designComponent="${designComponent}" />
	</c:when>
</c:choose>
