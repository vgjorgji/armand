...
<%@ taglib prefix="titles" uri="http://www.vcms.com/tags/component/titles"%>


<c:choose>
	<c:when test="${designComponent.componentStyle eq 'Learn'}">
		<titles:learn website="${website}" websiteView="${websiteView}" page="${page}" designComponent="${designComponent}" />
	</c:when>
</c:choose>
