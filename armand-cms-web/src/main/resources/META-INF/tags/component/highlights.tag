...
<%@ taglib prefix="highlights" uri="http://www.vcms.com/tags/component/highlights"%>


<c:choose>
	<c:when test="${designComponent.componentStyle eq 'Learn'}">
		<highlights:learn website="${website}" websiteView="${websiteView}" page="${page}" designComponent="${designComponent}" />
	</c:when>
</c:choose>
