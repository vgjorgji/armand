...
<%@ taglib prefix="jumbotron" uri="http://www.vcms.com/tags/component/jumbotron"%>


<c:choose>
	<c:when test="${designComponent.componentStyle eq 'Learn'}">
		<jumbotron:learn website="${website}" websiteView="${websiteView}" page="${page}" designComponent="${designComponent}" />
	</c:when>
</c:choose>
