...
<%@ taglib prefix="medias" uri="http://www.vcms.com/tags/component/medias"%>


<c:choose>
	<c:when test="${designComponent.componentStyle eq 'Learn'}">
		<medias:learn website="${website}" websiteView="${websiteView}" page="${page}" designComponent="${designComponent}" />
	</c:when>
</c:choose>
