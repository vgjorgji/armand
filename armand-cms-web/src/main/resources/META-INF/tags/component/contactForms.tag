...
<%@ taglib prefix="contactForms" uri="http://www.vcms.com/tags/component/contactForms"%>


<c:choose>
	<c:when test="${designComponent.componentStyle eq 'Learn'}">
		<contactForms:learn website="${website}" websiteView="${websiteView}" page="${page}" designComponent="${designComponent}" />
	</c:when>
</c:choose>
