...
<%@ taglib prefix="contactInfos" uri="http://www.vcms.com/tags/component/contactInfos"%>


<c:choose>
	<c:when test="${designComponent.componentStyle eq 'Learn'}">
		<contactInfos:learn website="${website}" websiteView="${websiteView}" page="${page}" designComponent="${designComponent}" />
	</c:when>
</c:choose>
