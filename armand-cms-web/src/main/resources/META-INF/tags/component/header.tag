...
<%@ taglib prefix="header" uri="http://www.vcms.com/tags/component/header"%>


<c:choose>
	<c:when test="${designComponent.componentStyle eq 'Learn'}">
		<header:learn website="${website}" websiteView="${websiteView}" page="${page}" designComponent="${designComponent}" />
	</c:when>
</c:choose>
